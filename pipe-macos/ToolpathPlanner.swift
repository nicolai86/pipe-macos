...
struct ToolpathPlanner {
    ...
    func plan(
        ...
        for i in 0..<finalPath.count - 1 {
            signedArea +=
                (finalPath[i].x * (finalPath[i + 1].a * k)
    ...
    private func buildLeadOut(
        ...
        case .rotationalArc:
            return buildRotationalArcLeadOut(
                path: path,
                k: k,
                config: config,
                isScrapLeft: isScrapLeft
    ...
    private func buildRotationalArcLeadOut(
        ...
        for i in 1...steps {
            ...
            pts.append(
                ToolpathPoint(
                    x: pLast.x,
                    a: pLast.a + sideMultiplier * sweepDeg * t
                )
            )
        }
        return pts
    ...
    /// Helical sweep: torch rotates A while diagonally ramping X from the scrap zone.
    /// Used for sever cuts so the torch clears protruding features (like hooks)
    /// and leaves no witness mark on the cut face.
    private func buildRotationalArcLeadIn(
        ...
        // 1. Determine Scrap Direction
        let scrapDirection: CGFloat
        if featureType == .startCut {
            scrapDirection = -1.0  // Scrap is in the -X direction
        } else if featureType == .endCut {
            scrapDirection = 1.0  // Scrap is in the +X direction
        } else {
            scrapDirection = 0.0  // Fallback for internal features
        }
        ...
        for i in 0..<steps {
            let t = CGFloat(i) / CGFloat(steps)  // t goes from 0.0 to 0.8

            // Interpolate both axes simultaneously for a smooth 3D diagonal ramp
            let currentA = entryA + sideMultiplier * leadAngleDeg * t
            let currentX = startX + (p0.x - startX) * t

            leadIn.append(ToolpathPoint(x: currentX, a: currentA))
        }
        // At t=1.0, the point is exactly p0, which is handled naturally
        // because p0 is the first point of the returned cutPath array.
        return (leadIn, path)
    }
    ...
    private func buildTangentArcLeadIn(
        ...
        let startAngle =
            (gamma - sideMultiplier * .pi / 2.0) - sideMultiplier * leadTheta
    ...
    private func buildCenterPierceLeadIn(
        ...
        for i in 0..<steps {
            let t = CGFloat(i) / CGFloat(steps)
            leadIn.append(
                ToolpathPoint(x: cx + t * (p0.x - cx), a: ca + t * (p0.a - ca))
            )
        }
        return (leadIn, path)
    }
    ...
    struct HSSGeodesicProfile {
        ...
        private func arcLengthFolded(_ aN: CGFloat) -> CGFloat {
            ...
            while lo < hi {
                let m = (lo + hi) / 2
                if table[m].a < aN { lo = m + 1 } else { hi = m }
            }
        ...
        private func angleFolded(_ sN: CGFloat) -> CGFloat {
            ...
            while lo < hi {
                let m = (lo + hi) / 2
                if table[m].s < sN { lo = m + 1 } else { hi = m }
            }
